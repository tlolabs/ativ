...
The native workflow uses macOS 15 ARM64/Intel, Windows 2025 x64, Windows 11 ARM64, and Ubuntu 24.04 x64/ARM64 builders. Windows uses MSYS2 CLANG64/CLANGARM64, with native target compilers and static library builds. MSYS2 provides build tools only; it does not supply FFmpeg. macOS uses Xcode Clang and the macOS 13 deployment target; Linux uses GCC and the Ubuntu 24.04 glibc baseline.

The cache key hashes the dependency record (including FFmpeg version/checksum/configuration and external pins), recipe scripts, release key, target, and actual compiler/toolchain/SDK/runner inputs. There are no partial restore keys. Restored payloads are revalidated before use. Unrelated Rust/UI edits reuse a matching runtime. CI's `clean_ffmpeg: true` bypasses cache restoration and saving and rebuilds all source. A failed or damaged cache fails verification; use a clean dispatch to replace the build input. Neither signing nor notarization gates manual unsigned qualification.
