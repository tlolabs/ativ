use crate::{
    AvidError, MAX_OUTPUT_DIMENSION, MAX_OUTPUT_PIXELS, MAX_SOURCE_IMAGE_DIMENSION,
...
pub fn probe_audio_duration(tools: &MediaTools, path: &Path) -> Result<Option<f64>> {
    ...
    let output = Command::new(&tools.ffprobe)
        ...
        .stdin(Stdio::null())
        .output()
        .map_err(|error| AvidError::io("inspect the audio file", error))?;
    if !output.status.success() {
        return Err(AvidError::MediaToolFailed {
            tool: "ffprobe",
...
pub fn render_preview(
    ...
    let parent = output
        .parent()
        .ok_or_else(|| AvidError::InvalidInput("Choose a valid preview destination.".into()))?;
    fs::create_dir_all(parent)
        .map_err(|error| AvidError::io("create the preview folder", error))?;
    ...
    let result = Command::new(&tools.ffmpeg)
        ...
        .stderr(Stdio::piped())
        .output()
        .map_err(|error| AvidError::io("start FFmpeg for the preview", error))?;
...
pub fn render_video(
    ...
    if cancelled.load(Ordering::SeqCst) {
        return Err(AvidError::Cancelled);
    }
    ...
    let output_parent = request
        .output
        .parent()
        .ok_or_else(|| AvidError::InvalidInput("Choose a valid output destination.".into()))?;
    fs::create_dir_all(output_parent)
        .map_err(|error| AvidError::io("create the output folder", error))?;
    ...
    let mut child = command
        .spawn()
        .map_err(|error| AvidError::io("start FFmpeg", error))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| AvidError::MediaToolFailed {
            tool: "FFmpeg",
    ...
    let status = child
        .wait()
        .map_err(|error| AvidError::io("finish the FFmpeg process", error))?;
    if !status.success() {
        return Err(AvidError::MediaToolFailed {
            tool: "FFmpeg",
            detail: clean_failure(
    ...
    loop {
        ...
        match receiver.recv_timeout(Duration::from_millis(100)) {
            ...
            Err(mpsc::RecvTimeoutError::Timeout) => {
                if child
                    .try_wait()
                    .map_err(|error| AvidError::io("check FFmpeg", error))?
                    .is_some()
                {
                    return Ok(());
                }
...
fn validate_request(request: &RenderRequest) -> Result<()> {
    ...
    if request.fps == 0 || request.fps > 240 {
        return Err(AvidError::InvalidInput(
...
fn validate_file(path: &Path, kind: &str) -> Result<()> {
    if path.is_file() {
        ...
        Err(AvidError::InvalidInput(format!(
...
fn validate_dimensions(width: u32, height: u32) -> Result<()> {
    ...
    if width > MAX_OUTPUT_DIMENSION || height > MAX_OUTPUT_DIMENSION || pixels > MAX_OUTPUT_PIXELS {
        return Err(AvidError::InvalidInput(
            "The requested output exceeds A.V.I.D.'s 8K safety limit.".into(),
        ));
    }
    Ok(())
}
...
fn validate_image(tools: &MediaTools, image: &Path) -> Result<()> {
    ...
    let output = Command::new(&tools.ffprobe)
        ...
        .stdin(Stdio::null())
        .output()
        .map_err(|error| AvidError::io("inspect the image", error))?;
    if !output.status.success() {
        return Err(AvidError::MediaToolFailed {
            tool: "ffprobe",
    ...
    let (width, height) = dimensions.trim().split_once('x').ok_or_else(|| {
        AvidError::InvalidInput("The selected file does not contain a readable image.".into())
    })?;
    let width = width.parse::<u32>().map_err(|_| {
        AvidError::InvalidInput("The selected image dimensions are invalid.".into())
    })?;
    ...
    {
        return Err(AvidError::InvalidInput(
            "The source image exceeds A.V.I.D.'s 50-megapixel safety limit.".into(),
        ));
    }
    Ok(())
}
...
fn paths_refer_to_same_file(left: &Path, right: &Path) -> Result<bool> {
    ...
    let left_meta =
        fs::metadata(left).map_err(|error| AvidError::io("inspect an input file", error))?;
    let right_meta =
        fs::metadata(right).map_err(|error| AvidError::io("inspect the output file", error))?;
    #[cfg(unix)]
    {
...
fn publish(staged: &Path, output: &Path) -> Result<()> {
    fs::rename(staged, output).map_err(|error| AvidError::io("publish the completed output", error))
}
...
fn publish(staged: &Path, output: &Path) -> Result<()> {
    if !output.exists() {
        return fs::rename(staged, output)
            .map_err(|error| AvidError::io("publish the completed output", error));
    }
    let backup = staging_path(output, "recovery", "bak");
    fs::rename(output, &backup)
        .map_err(|error| AvidError::io("protect the previous output", error))?;
    match fs::rename(staged, output) {
        ...
        Err(error) => {
            ...
            Err(AvidError::io("publish the completed output", error))
        }
    }
}
