...
fn locate_tool(name: &'static str, override_path: Option<PathBuf>) -> Result<PathBuf> {
    ...
    let executable =
        env::current_exe().map_err(|error| AtivError::io("locate the A.T.I.V. engine", error))?;
    ...
    Err(AtivError::MediaToolsUnavailable(format!(
        "A.T.I.V. could not find its bundled {name} binary. Reinstall the application or restore the application package."
    )))
}
...
fn validate_tool(path: &Path, name: &'static str) -> Result<String> {
    let output = Command::new(path)
        ...
        .stdin(Stdio::null())
        .output()
        .map_err(|error| AtivError::io("start a bundled media tool", error))?;
    if !output.status.success() {
        return Err(AtivError::MediaToolsUnavailable(format!(
            "The bundled {name} binary could not run. Reinstall A.T.I.V."
        )));
    }
    ...
    if !first_line.to_ascii_lowercase().starts_with(name) {
        return Err(AtivError::MediaToolsUnavailable(format!(
            "The bundled file named {name} did not identify itself correctly. Reinstall A.T.I.V."
        )));
    }
    Ok(first_line)
}
