...
public sealed class EngineClient
{
    ...
    private static string EnginePath
    {
        get
        {
            var configured = Environment.GetEnvironmentVariable("AVID_ENGINE_PATH");
            if (!string.IsNullOrWhiteSpace(configured)) return configured;
            var sidecar = Path.Combine(AppContext.BaseDirectory, "avid-engine.exe");
            if (File.Exists(sidecar)) return sidecar;
            throw new FileNotFoundException("The A.V.I.D. media engine is missing. Reinstall the application.", sidecar);
        }
    }
